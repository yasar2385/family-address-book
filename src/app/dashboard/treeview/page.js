...
export default function FamilyTreeView() {
    ...
    const buildFamilyTree = (members, relations) => {
        ...
        relations.forEach((relation) => {
            ...
            // Father and Mother (parent-to-child)
            if (relationType === "Father" || relationType === "Mother") {
                ...
                tree[member2Id].parents.push({
                    id: member1Id,
                    name: tree[member1Id].name,
                    relation: relationType
                });
            }
            // Son and Daughter (child-to-parent)

            if (relationType === "Son" || relationType === "Daughter") {
                tree[member1Id].parents.push({
                    id: member2Id,
                    name: tree[member2Id].name,
                    relation: relationType
                });
                tree[member2Id].children.push({
                    id: member1Id,
                    name: tree[member1Id].name,
                    relation: relationType
                });
                    ...
                    <div className="children flex flex-row justify-around mt-4 space-x-8">
                        {member.children.map((child) => (
                            <div key={child.id} className="relative flex flex-col items-center">
                                ...
                                {renderTree(tree, child.id)} {/* Recursively render children */}
